...
require_once __DIR__ . '/../libs/functions.php';
	...
	class Tibber extends IPSModule
	{
		...
		private function ProcessPriceData(string $result)
		{
			...
			// Consider BOTH today and tomorrow to avoid midnight edge-cases where today has <2 items.
			...
			} elseif (is_array($tomArr) && count($tomArr) >= 2) {
				$dt = strtotime($tomArr[1]['startsAt']) - strtotime($tomArr[0]['startsAt']);
				if ($dt > 0 && $dt <= 1200) { $useQuarter = true; }
			}
			if ($useQuarter) {
			...
			if ($useQuarter) {
                ...
                // keep existing quarter-hour cache; do not clear to preserve continuity across midnight/transient issues
